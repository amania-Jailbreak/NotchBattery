...
- (void)viewDidLoad {
	...
	self.batteryView.image = [[UIImage systemImageNamed:@"battery.100.bolt"] imageWithTintColor:[UIColor systemGreenColor]];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
	if (section == [self numberOfGroups] - 1) {
		...
		NSString *primary = @"Notch battery";
		NSString *secondary = @"v1.0.0 © amaniaTools";
	...
	if (indexPath.section == 6) {
		if (indexPath.row == 0) {
			...
			if ([SFSafariViewController class] != nil) {
				SFSafariViewController *safariView = [[SFSafariViewController alloc] initWithURL:[NSURL URLWithString:@"https://x.com/amania_jp"]];
